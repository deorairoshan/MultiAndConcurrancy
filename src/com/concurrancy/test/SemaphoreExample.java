...
public class SemaphoreExample {
	...
	private static class PrinterQueue {
		// This Semaphore will keep track of no. of printers used at any point
		// of time.
		private final Semaphore semaphore;

		// While checking/acquiring a free printer out of three available
		// printers, we will use this lock.
		...
		public void printJob(Object document) {
			try {
				...
				// Printing is done; Free the printer to be used by other
				// threads.
		...
		private int getPrinter() {
			int foundPrinter = -1;
			try {
				// Get a lock here so that only one thread can go beyond this at
				// a time
				printerLock.lock();
