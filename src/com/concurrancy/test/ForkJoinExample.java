...
public class ForkJoinExample {
	...
	private static class FolderProcessor extends RecursiveTask<List<String>> {
		...
		// This attribute will store the full path of the folder this task is
		// going to process.
		...
		// Implement the compute() method. As you parameterized the
		// RecursiveTask class with the List<String> type,
		// this method has to return an object of that type.
		@Override
		protected List<String> compute() {
			// List to store the names of the files stored in the folder.
			List<String> list = new ArrayList<>();
			// FolderProcessor tasks to store the subtasks that are going to
			// process the subfolders stored in the folder
			List<FolderProcessor> tasks = new ArrayList<>();
			// Get the content of the folder.
			File file = new File(path);
			File content[] = file.listFiles();
			// For each element in the folder, if there is a subfolder, create a
			// new FolderProcessor object
			...
			if (content != null) {
				for (int i = 0; i < content.length; i++) {
					...
					// extension you are looking for using the checkFile()
					// method
					// and, if they are equal, store the full path of the file
					// in the list of strings declared earlier.
					else {
		...
		// For each task stored in the list of tasks, call the join() method
		// that will wait for its finalization and then will return the result
		// of the task.
